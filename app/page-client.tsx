...
import { organizationJsonLd } from "@/lib/site-config"
import { withPublicTimeout } from "@/lib/public-data"
...
export default function HomePage() {
  const { data: stats, error: statsError, mutate: refreshStats } = useSWR("public-stats", () => withPublicTimeout(publicApi.stats(), "Thống kê public"), {
    revalidateOnFocus: false,
  })
  const { subjects, grades, locations, teachingModes, error: masterDataError, isLoading: masterDataLoading, refresh: refreshMasterData } = useMasterDataCatalog()
  const { tutors, isLoading: tutorsLoading, error: tutorsError, refresh: refreshTutors } = useTutors({
      ...
      <main className="flex-1">
        <section className="gradient-mesh relative overflow-hidden pb-12 pt-14 md:pb-16 md:pt-20">
          <div className="premium-container">
              ...
              <div className="glass-card-strong content-fade-up overflow-hidden rounded-2xl">
                ...
                <div className="stagger-list space-y-3 p-4">
                  {featuredTutors.length ? featuredTutors.map((tutor) => {
                    const tutorSubjects = tutor.subjects?.slice(0, 2).join(", ") || "Môn học đang cập nhật"
                    const tutorRating = Number.isFinite(tutor.rating) ? tutor.rating.toFixed(1) : "Mới"
                    const tutorResponseRate = Number.isFinite(tutor.responseRate) ? `${tutor.responseRate}% phản hồi` : "Đang cập nhật"
                    return (
                      <Link key={tutor.id} href={`/tutors/${tutor.id}`} className="item-row stagger-item flex items-center justify-between gap-3 rounded-xl">
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="truncate text-sm text-slate-600">{tutorSubjects}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-primary">{tutorRating}</p>
                          <p className="text-xs text-slate-500">{tutorResponseRate}</p>
                        </div>
                      </Link>
                    )
                  }) : tutorsLoading ? (
                    ...
                    <div className="soft-panel space-y-3 p-5 text-sm text-slate-600">
                      <p>Không tải được gia sư nổi bật từ backend.</p>
                      <Button type="button" variant="outline" size="sm" onClick={() => refreshTutors()}>
                        Thử lại
                      </Button>
                    </div>
                  ) : (
                    <div className="soft-panel space-y-3 p-5 text-sm text-slate-600">
                      <p>Chưa có gia sư nổi bật phù hợp.</p>
            ...
            {(statsError || masterDataError || masterDataLoading) && (
              <PublicDataNotice
                className="mx-auto mt-8 max-w-4xl"
                isLoading={masterDataLoading && !statsError && !masterDataError}
                loadingMessage="Đang tải danh mục từ backend. Search vẫn có thể dùng dữ liệu dự phòng."
                message={
                  statsError
                    ? "Thống kê public đang dùng số liệu dự phòng vì backend chưa phản hồi."
                    : "Một số danh mục đang dùng dữ liệu dự phòng vì backend chưa phản hồi."
                }
                onRetry={() => {
                  refreshStats()
                  refreshMasterData()
          ...
          <div className="premium-container">
            ...
            <div className="stagger-list grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {featuredTutors.length ? featuredTutors.map((tutor) => (
                ...
                <div className="glass-card-strong rounded-2xl p-6 text-sm text-muted-foreground md:col-span-2 lg:col-span-3">
                  {tutorsLoading ? "Đang tải hồ sơ gia sư nổi bật..." : "Chưa có hồ sơ gia sư nổi bật. Phụ huynh vẫn có thể gửi nhu cầu để được tư vấn matching thủ công."}
                </div>
              )}
            </div>
