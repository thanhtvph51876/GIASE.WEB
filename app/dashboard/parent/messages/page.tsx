...
export default function ParentMessagesPage() {
  ...
  return (
    <div className="space-y-5">
      <div className="surface-panel flex flex-col gap-3 border-l-4 border-l-primary p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-950">Tin nhắn phụ huynh</h1>
          <p className="text-muted-foreground">Trao đổi với gia sư và đội ngũ hỗ trợ theo lịch học, booking hoặc lớp của con.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" asChild><Link href="/tutors">Tìm gia sư</Link></Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 p-3">
            {conversations.length ? conversations.map((conversation) => (
              ...
              >
                <b>{conversation.participantNames.join(" · ") || "Hội thoại"}</b>
                <p className="truncate opacity-80">{conversation.lastMessage || "Chưa có tin nhắn"}</p>
              </button>
            )) : (
              <div className="rounded-lg border border-dashed bg-slate-50 p-5 text-center text-sm text-muted-foreground">
                <p className="font-medium text-slate-800">Chưa có cuộc trò chuyện nào.</p>
                <p className="mt-1">Khi có booking/lớp hoặc cần hỗ trợ, hội thoại sẽ xuất hiện tại đây.</p>
                <Button asChild size="sm" className="mt-3"><Link href="/dashboard/parent/schedule">Xem lịch học</Link></Button>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="flex h-[560px] flex-col p-4">
            <div className="flex-1 space-y-3 overflow-y-auto">
              {messages.length ? messages.map((message) => (
                ...
                <div className="flex min-h-full items-center justify-center rounded-lg border border-dashed bg-slate-50 p-4 text-center text-sm text-muted-foreground">
