    ...
    <div className="premium-page-bg flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <section className="gradient-mesh py-16 text-center">
          <h1 className="font-heading text-4xl font-bold text-slate-950">Liên hệ Gia Sư Sư Phạm</h1>
        ...
        <section className="app-container grid gap-6 py-10 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-4">
            {[{ icon: Mail, label: "Email hỗ trợ", value: "hotro@giasusupham.vn" }, { icon: Phone, label: "Hotline", value: "0901 234 567" }, { icon: MapPin, label: "Địa chỉ", value: "280 An Dương Vương, Quận 5, TP.HCM" }].map((item) => <Card key={item.label} className="glass-card rounded-2xl"><CardContent className="flex gap-3 p-5"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><item.icon className="h-5 w-5" /></div><div><p className="font-semibold text-slate-900">{item.label}</p><p className="text-sm text-muted-foreground">{item.value}</p></div></CardContent></Card>)}
          </div>
          <Card className="glass-card-strong rounded-2xl">
            ...
            <CardContent className="space-y-4">
              ...
              <Button className="rounded-full" onClick={submit} disabled={isSubmitting}>{isSubmitting ? "Đang gửi..." : "Gửi liên hệ"}</Button>
            </CardContent>
          </Card>
        </section>
      </main>
