...
| `tutor_admin` | Vận hành gia sư, matching, lớp/buổi học, CRM tutor | Tutors, verifications, requests, bookings, classes, sessions, CRM |
| `support_admin` | CSKH và hỗ trợ vận hành | Users read, contacts, messages, notifications, review read, complaints, CRM |
...
| Khiếu nại | `/admin/complaints` | `operations.read` | `complaints.manage` |
| CRM học sinh | `/admin/students/[id]` | `users.read` | `crm.manage` cho note/risk |
| CRM phụ huynh | `/admin/parents/[id]` | `users.read` | `crm.manage` cho note/risk |
| CRM gia sư | `/admin/tutors/[id]` | `tutors.read` | `crm.manage` cho note/risk |
...
| `support_admin` | Users, requests, bookings, classes, sessions, messages, notifications, contacts, reviews, complaints, reports, operations | Gửi notification, xử lý contact, complaint, CRM note/risk |
...
| `PATCH/POST /admin/disputes/{id}/*` | `complaints.manage` |
| `GET /admin/users/{id}/crm` | `users.read` |
| `GET /admin/tutors/{id}/crm` | `tutors.read` |
| `POST /admin/users/{id}/notes`, `/risk-flags` | `crm.manage` |
| `POST /admin/tutors/{id}/notes`, `/risk-flags` | `crm.manage` |
...
- FE/BE đã đồng bộ `complaints.manage` cho case management và `crm.manage` cho note/risk flag.
- FE đã bỏ quyền suspend khỏi `tutor_admin` để khớp backend: tutor admin duyệt/từ chối/yêu cầu bổ sung, không khóa/mở tutor.
...
- Nếu cần finance/verification đọc CRM sâu thì phải thêm permission riêng; hiện tại finance/verification không được `crm.manage`.
